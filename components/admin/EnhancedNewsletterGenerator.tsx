...
export function EnhancedNewsletterGenerator() {
  ...
  const [showSubscriberManagement, setShowSubscriberManagement] = useState(false);
  ...
  const [subscriberFilter, setSubscriberFilter] = useState<'all' | 'active' | 'inactive'>('active');
  ...
  const loadSubscribers = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Auto-select all active subscribers
        const activeSubscriberIds = data.subscribers
          ?.filter((sub: Subscriber) => sub.status === 'active')
          .map((sub: Subscriber) => sub._id) || [];
  ...
  const handleSendNewsletter = async () => {
    ...
    // Validate recipients
    const validManualEmails = manualEmails.filter(email => email.trim() && email.includes('@'));
    const selectedSubscriberEmails = subscribers
      .filter(sub => selectedSubscribers.includes(sub._id))
      .map(sub => sub.email);

    const allRecipients = [...selectedSubscriberEmails, ...validManualEmails];

    if (allRecipients.length === 0) {
      toast({
        title: "No Recipients",
        description: "Please select subscribers or add manual email addresses",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Newsletter sent to ${data.stats?.successfulSends || 0} recipients successfully`,
      });
    } catch (error) {
  ...
  const updateManualEmail = (index: number, email: string) => {
    setManualEmails(prev => prev.map((em, i) => i === index ? email : em));
  };
  ...
  const getFilteredSubscribers = () => {
    ...
    if (subscriberFilter !== 'all') {
      filtered = filtered.filter(sub => sub.status === subscriberFilter);
    }
  ...
  const validManualEmails = manualEmails.filter(email => email.trim() && email.includes('@'));
  const totalRecipients = selectedCount + validManualEmails.length;
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select subscribers and add manual email addresses for your newsletter
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">Subscribers ({selectedCount} selected)</span>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                    ...
                    <Button
                      ...
                      disabled={isLoadingSubscribers}
                    >
                      <RefreshCw className={`h-4 w-4 ${isLoadingSubscribers ? 'animate-spin' : ''}`} />
                    </Button>
                  </div>
                </div>

                {/* Subscriber Search and Filter */}
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  {isLoadingSubscribers ? (
                    ...
                    <div className="divide-y">
                      {filteredSubscribers.map((subscriber) => (
                        <div key={subscriber._id} className="p-3 flex items-center gap-3 hover:bg-gray-50">
                          <Checkbox
                            checked={selectedSubscribers.includes(subscriber._id)}
                            onCheckedChange={() => toggleSubscriberSelection(subscriber._id)}
                          />
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">Manual Email Addresses ({validManualEmails.length} valid)</span>
                </div>
